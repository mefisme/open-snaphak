 ...
 * we leave the input handling fully intact and detour two FSM LEAVES, each gated on g_wire_mode:
 *
 ...
 *
 * The tool-struct field offsets below are read straight out of the two leaves' own decompile (the values
...
#define SIG_OUTPUT_CREATOR   "ConnectOutputCreator"   /* 0xcdbb40 -- Hook 2: the connect creator        */
...
#define TOOL_SLOT1_OFF       0x14   /* chain slot 1 = target (the trailing finalize lays slot0->slot1) */
...
static output_creator_fn g_orig_output_creator = NULL;  /* trampoline to the original connect creator    */
...
void sh_wiring_mode_install(const uint8_t *module_base)
{
    ...
    if (!wm_resolve_sig(SIG_OUTPUT_CREATOR, &rc) || rc.status != SIG_OK) {
        backend_log("B2: sh_target_any wire-any NOT armed (connect-creator sig unresolved)");
        return;
    }
    ...
    /* Install both (off-by-default) detours. If the second fails, roll the first back -- no half-state. */
    ...
    if (tramp_creator == NULL) {
        ...
        g_reset = NULL;
        backend_log("B2: sh_target_any wire-any NOT armed (connect-creator detour install failed)");
        return;
    }

    g_orig_output_state   = (output_state_fn)tramp_state;
    g_orig_output_creator = (output_creator_fn)tramp_creator;
    backend_log("B2: sh_target_any wire-any ready (off by default; output-select + connect-creator detours installed)");
}

void h_wiring_mode(struct idCmdArgs *a)
{
    (void)a;
    if (g_orig_output_state == NULL || g_orig_output_creator == NULL) {
